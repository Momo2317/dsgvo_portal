...
export default function BillingPage() {
      ...
      if (sessionId) {
        try {
          ...
          if (res.ok && data?.status === 'active') {
            setSuccessMsg(
              data.plan
                ? `Ihr ${String(data.plan).charAt(0).toUpperCase() + String(data.plan).slice(1)}-Plan wurde aktiviert!`
                : 'Ihr Abonnement wurde erfolgreich aktiviert!'
            );
          } else if (!res.ok) {
            setErrorMsg(data?.error || 'Zahlung konnte nicht bestätigt werden.');
          }
        } catch {
          setErrorMsg('Zahlung konnte nicht bestätigt werden. Bitte Seite neu laden.');
        }
  ...
  const handleManageSubscription = async () => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Abonnement-Verwaltung konnte nicht geöffnet werden.');
    ...
    <DashboardLayout>
      <DashboardPage width="content">
        <PageHeader
          ...
          description="Verwalten Sie Ihren Plan, Zahlungsmethode und Kündigung."
        />
          ...
          {loading ? (
            ...
            <>
              {subscription && (
                <div className="mb-6 bg-primary/5 border border-primary/20 rounded-xl px-5 py-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                      <p className="text-xs font-semibold text-primary mb-0.5">Aktueller Plan</p>
                      <p className="text-sm font-bold text-foreground capitalize">
                        {subscription.plan} ·{' '}
                        {subscription.billingInterval === 'monthly' ? 'Monatlich' : 'Jährlich'}
                      </p>
                    ...
                    <div className="flex flex-col sm:items-end gap-2">
                      <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-accent/10 text-accent border border-accent/20 w-fit">
                        {subscription.cancelAtPeriodEnd ? 'Gekündigt' : 'Aktiv'}
                      </span>
                      <button
                        onClick={handleManageSubscription}
                        disabled={portalLoading}
                        className="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold border border-border rounded-lg hover:bg-muted transition-all disabled:opacity-60"
                      >
                        {portalLoading ? (
                  ...
                  <p className="text-xs text-muted-foreground mt-3">
                    Über Stripe können Sie Ihre Zahlungsmethode ändern oder das Abonnement zum Periodenende kündigen.
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {PLAN_CONFIG.map((plan) => {
                  ...
                  return (
                    <div
                      key={plan.id}
                      className={`relative ui-card flex flex-col ${
                        plan.highlight
